...
# Is this function redundant with the function two_proteins_same_gene_in_gff3_2
# below?
def get_overlapping_genes_from_gff3(sql_database,
    ...
    if just_gene:
        ...
        # Get gene IDs (or IDs for whatever records are found to overlap with
        # the cluster range).
        for gene in overlapping_entries:
            overlap_ids.append(gene.id)
            #try:
...
def cds_record_matches_prot_id(cds_obj, prot_id):
    ...
    statements.
    """
    print("""\t\t\t\tStarting cds_record_matches_prot_id function.""")
    ...
    if not prot_id_associated_with_cds_obj:
        try:
    ...
    print("""\t\t\t\tEnding cds_record_matches_prot_id function.""")
...
def two_proteins_same_gene_in_gff3_2(sql_database, prot_id_1, prot_id_2):
    ...
    to the same gene.
    """
    print("""\t\t\tStarting two_proteins_same_gene_in_gff3_2 function.""")
    # Parse the SQL database that was generated by gffutils for the relevant
    # GFF3 file.
    db = gffutils.FeatureDB(sql_database, keep_order=True)
    ...
    print("""\t\t\tEnding two_proteins_same_gene_in_gff3_2 function.""")
...
def two_proteins_same_gene_in_gff3_3(sql_database, prot_id_1, prot_id_2):
    ...
    try:
        ...
        assert 2!=2, """Error: Failed to parse SQL file at path %s""" % sql_database
...
def count_paralogues3(csv_file,
    ...
    for combo in index_dict.keys():
        ...
        if len(ranked_seq_objs) >= 1:
            ...
            # Add sequence comparison info to dict.
            #print("""Add sequence comparison info to dict.""")
            full_redundant_gene_model_dict =\
