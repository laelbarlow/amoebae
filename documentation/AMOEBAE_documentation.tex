...
\subsection{Why AMOEBAE?}
    ...
    Webservices such as those provided by NCBI (URL*****) provide a means to
    ...
    %Although scripts in the AMOEBAE toolkit perform many operations more
    %efficiently than can be done without scripts, these are not applicable to
    %high-throughput genome-scale analysis as they do not constitute an
    %automated pipeline, and do not provide the efficiency necessary to run
    %analyses with hundreds of protein queries simultaneously. AMOEBAE is
    %instead a toolkit of scripts designed to provide the flexibility required
    %for detailed analysis of specific protein families, by performing specific
    %operations you may wish to include in your similarity searching workflow.
    %Therefore, the following documentation assumes that if you choose to use
    %AMOEBAE then you are using 
...
%\subsection{Suggestions for new bioinformaticians}
%
%This assumes that you have some experience running similarity searches using
%BLAST on webservices such as NCBI.
%
%To use AMOEBAE effectively, you will need the following skills:
%
%a basic understanding of commandline usage on macOS or Linux
%    Link to command-line cheatsheet.
%
%know how to install software. 
%
%Some basic understanding of how to run scripts written in various languages.
%
%If you are new to using scripts for comparative genomics
%analysis, a simpler Jupyter Notebook is included: XXXXXXlink. 
